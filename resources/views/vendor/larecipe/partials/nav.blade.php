<div class="fixed pin-t pin-x z-40">
    ...
    <nav class="flex items-center justify-between text-black bg-navbar shadow-xs h-16">
        <div class="flex items-center flex-no-shrink">
            <a href="{{ url('/inicio') }}" class="flex items-center flex-no-shrink text-black mx-4">
            ...
            @auth
                ...
                <larecipe-dropdown>
                    <larecipe-button type="white" class="ml-2">
                        {{ auth()->user()->rut ?? 'Account' }} <i class="fa fa-angle-down"></i>
                    </larecipe-button>
                    ...
                    <template slot="list">
                        <form action="/logout" method="POST">
                            ...
                            <button type="submit" class="py-2 px-4 text-white bg-danger inline-flex"><i class="fa fa-power-off mr-2"></i> Cerrar sesión</button>
                        </form>
                    </template>
                </larecipe-dropdown>
                {{-- /account --}}
