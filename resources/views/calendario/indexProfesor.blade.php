                        ...
                        <div class="scrollable">
                            ...
                            <table class="table table-bordered" id="my-table">
                                ...
                                <tr>
                                    @if($primerlunes!=1)
                                        @for($i=7; $i>0; $i--)
                                            <?php $calendarios= Calendario::all()->where('fecha',date("Y-m-d", mktime(0, 0, 0, date('n'), $primerlunes-$i, date('Y'))))->where('id_profesor',$profesor->id)?>
                                            ...
                                            @endif   
                                                @foreach($calendarios as $calendario)
                                                    <span class="label pull-left {{$calendario->asignatura->color}} btn-block">{{$calendario->descripcion.' - '.$calendario->curso->nivel.'° '.$calendario->curso->grado.' '.$calendario->curso->letra}}</span> <br>
                                                @endforeach
                                            </td>
                                        @endfor
                                    @endif
                                ...
                                @for($k=1; $k<6; $k++)
                                    <tr>
                                        ...
                                        @for($l=$var; $l<38; $l++)
                                            ...
                                            <?php $calendarios= Calendario::all()->where('fecha',date("Y-m-d", mktime(0, 0, 0, date('n'), $l, date('Y'))))->where('id_profesor',$profesor->id)?>
                                            ...
                                            @endif
                                                @foreach($calendarios as $calendario)
                                                    <span class="label pull-left {{$calendario->asignatura->color}} btn-block">{{$calendario->descripcion.' - '.$calendario->curso->nivel.'° '.$calendario->curso->grado.' '.$calendario->curso->letra}}</span>
                                                @endforeach
                                            </td>
                    ...
                    <ul class="pagination pagination-sm no-margin pull-left">
                        <div class="row">
                            <div class="col-lg-12 col-lg-offset-1">
                                ...
                                @foreach($profesor->asignaturas->unique('nombre')->sortBy('nombre') as $asignatura)
